...
product_details, product where `order`.cus_id=2
...
from product_details group by supp_id having count(product_details.supp_id)>1)
...
inner join product on product.pro_id=product_details.pro_id inner join
category on category.cat_id=product.cat_id having min(`order`.ord_amount);
...
product_details on product_details.prod_id=`order`.prod_id inner join
product on product_details.pro_id=product.pro_id
...
select supplier.supp_name, supplier.supp_id, rating.rat_ratstars, customer.cus_name from supplier
...
*/
select sum(`order`.ord_amount) as amount from `order` 
inner join customer on `order`.cus_id=customer.cus_id where customer.cus_gender="m";
